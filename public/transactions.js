...
form.addEventListener("submit", async (event) => {
  ...
  const res = await fetch(`https://moneymind-hc1s.onrender.com/transactions`, {
    method: "POST",
    headers: {
...
document.addEventListener("click", async (e) => {
  if (e.target.classList.contains("btn-delete")) {
    ...
    if (confirm("Voulez-vous vraiment supprimer cette transaction ?")) {
      const res = await fetch(`https://moneymind-hc1s.onrender.com/transactions/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
      if (res.ok) {
...
if (resetFilterBtn) {
  resetFilterBtn.addEventListener("click", async () => {
    ...
    const res = await fetch("https://moneymind-hc1s.onrender.com/transactions", { credentials: "include" });
    if (res.ok) {
      const data = await res.json();
      renderTransactions(data.transactions || data);
    } else {
